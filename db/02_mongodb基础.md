...
- <font color=red>db.集合.insert({})</font>  
insert：_id存在会报错 save：_id存在会更新
#### <font color=gray>删</font>
- <font color=red>db.集合.remove({query}, {justOne:true})</font>  
justOne默认false删除所有,true只删除第一条  
- db.position.remove({gender:0}, {justOne:true})  
- db.position.remove({})  # 清空集合
#### <font color=gray>改</font>
- <font color=red>db.集合.update({query}, {update}, {multi:true})</font>  
query相当于where  
update相当于set  
multi默认false只更新第一条,true更新所有  
...
- db.集合.find({query})：查询全部  
- db.集合.findOne({query})：查询返回第一个  
- db.集合.find({query}).pretty()：格式化查询结果  
## mongodb高级查询  
比较运算符：  
默认是等于判断, $lt < ; $lte <= ; $gt > ; $gte >= ; $ne !=  
db.position.find({name:'gj'})：查询名字=gj的  
db.position.find({age:{$gte:18}})：查询年龄>=18的  
逻辑运算符：  
默认是逻辑与; $or 表示逻辑或  
db.position.find({age:{$gte:18},gender:1})：查询年龄>=18且性别=1的  
db.position.find({$or:[{age:{$gt:18}},{gender:1}]})：查询年龄>=18或性别=1的  
db.position.find({$or:[{age:{$gte:18}},{gender:1}],name:'gj'})：查询年龄>=18或性别=1且名字=gj的  
范围运算符：  
使用"$in"和"$nin"判断是否在某个范围内  
db.position.find({age:{$in:[18,28]}})：查询年龄为18/28的  
支持正则表达式：  
使用//或$regex编写正则表达式  
db.position.find({name:/^黄/})  
db.position.find({name:{$regex:'^黄'}})：查询姓黄的  
自定义查询：  
使用$where后面写一个函数返回满足query的数据  
db.position.find({$where:function(){return this.age>20}})：查询年龄>20的  
limit()：db.集合.find().limit(10)  
skip()：db.集合.find().skip(2)  -- 从第3条开始查询  
for(i=0;i<15;i++){db.t1.insert({_id:i})}  
db.position.find().limit(4).skip(5)  
db.position.find().skip(5).limit(4)  # limit和skip结合使用时不分先后顺序  
投影：返回指定key的文档查询结果  
db.集合.find({},{字段名称:1,...})  
-- 在第二个参数中将需要显示的字段设置为1即可,不设置就不显示;_id列默认是显示的,如果不想显示需要明确设置为0  
db.position.find({},{"name":1, "gender":1})  -- 如果要选取的字段很少就将需要的字段指定为1  
db.position.find({},{"_id":0, "age":0})  -- 如果要选取的字段太多可以将不需要的字段设为0  
排序：  
db.集合.find().sort.md({字段:1,...})  
db.position.find().sort.md({gender:-1,age:1})  # 1升序-1降序  
统计：  
db.集合.find({query}).count() 等价于 db.集合.count({query})  
db.position.find({gender:1}).count()  
db.position.count({age:{$gt:20},gender:1})  
去重：  
db.集合.distinct('去重字段',{query})  
db.position.distinct('gender',{age:{$gt:18}})  
## mongodb聚合操作：  
