...
def main01():
    ...
    # 调用start()创建线程并启动线程;该线程会运行target函数,函数运行结束该子线程结束
    t1.start()
    t2.start()
...
def main03():
    # target指定线程执行的函数代码,args指定调用函数时传递的参数
    t5 = threading.Thread(target=test05, args=(g_nums,))  # 注意args传递的是tuple
    t6 = threading.Thread(target=test06, args=(g_nums,))
    t5.start()
    time.sleep(1)
    t6.start()
...
def test07(count):
    ...
    for i in range(count):
        # 上锁：如果之前没上锁那么上锁成功,如果之前已经上锁了会堵塞直到锁释放;为了避免堵塞太久上锁的代码越少越好,只给存在资源竞争的代码上锁即可
        lock.acquire()
        g_num += 1
        # 解锁
        lock.release()
...
def main04():
    # 当循环次数足够大时,操作全局变量的资源竞争冲突就体现出来了,需要给线程加锁
