- [参考文档1](http://www.cnblogs.com/hpucode/p/5204871.html)
- [参考文档2](http://www.cnblogs.com/xd502djj/p/3799432.html)
## hdfs
```bash
# Client：客户端
1.将文件按block块切分
...
行式存储：把一行的数据值串在一起存储,然后再存储下一行(1,zhangsan,18;2,lisi,19;3,wangwu,20...)
        读数据会读取一整行有些冗余,写数据会一次性写入速度很快,适合写多读少,面向OLTP
列式存储：把一列的数据值串在一起存储,然后再存储下一列(1,2,3 zhangsan,lisi,wangwu 18,19,20 ...)
        读数据只会读取相关列避免全表扫描且无需维护索引,写数据要把一行数据拆成单列多次写入消耗更大,适合读多写少,面向OLAP
        由于同一列数据重复度很高且空值可以不存储,可以压缩数据节省空间
# 文件存储格式
TEXTFILE和SEQUENCEFILE基于行存储
ORC和PARQUET基于列存储
...
truncate table test
-- 查看表详细信息
desc formatted test / show create table test    
...
alter table test drop partition (dt>=20160101,dt<20170101)  # 删除多个分区   
-- 重命名表
...
-- 视图
create view v01 as select * from debit_info where dt=regexp_replace(date_sub(current_date,1),'-','')  
...
-- mysql数据导入
load data local infile '...' [replace] into table test  # 覆盖/追加 
-- mysql数据导出
select * from test into outfile '...' FIELDS TERMINATED BY ',' ENCLOSED BY '"' LINES TERMINATED BY '\n';  
-- hive数据导入
load data [local] inpath '...' [overwrite] into table t1 [partition(dt='..')]  # 本地(复制)/hdfs(剪切)  覆盖/追加    
insert overwrite/into table t1 [partition(dt=20200101)] select * from t2 where ...
...
select * from test distribute by id sort by id;  
2 5 6 9 1 3 8    
...
lines terminated by '\n'  
-- ORC：先将数据按行分块再按列存储,保证同一条记录在一个块上避免读取多个数据块
...
set hive.exec.dynamic.partition.mode=nonstrict;        -- 设置插入模式 
set hive.optimize.sort.dynamic.partition=true;         -- 设置排序优化 
