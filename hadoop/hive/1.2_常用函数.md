...
hive> select split(concat_ws(',',collect_list(column)),',');  
-- sort_array：对数组元素排序
hive> select split(concat_ws(',',sort_array(collect_list(column))),',');  
-- lateral view explode：列转行,通常跟在表名后面
...
hive> select case when month(current_date) in (1,2,3) then add_months(trunc(current_date,'YY'),-3)
                  ...
                  when month(current_date) in (10,11,12) then add_months(trunc(current_date,'YY'),6)
             end;
hive> select case when month(current_date) in (1,2,3) then date_sub(trunc(current_date,'YY'),1)
                  ...
                  when month(current_date) in (10,11,12) then date_sub(add_months(trunc(current_date,'YY'),9),1)
             end;
-- 去年今天/第一天/最后一天
...
from 
    (
        select uid,sale,
               ntile(10) over(order by sale desc) as n
        from orders
    ) t 
where t.n=1;
...
from 
    (
        select num,
               ntile(100) over(order by num desc) as n
        from data
    ) t
where t.n=1;
